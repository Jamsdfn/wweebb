...
- Class 表达式
  ...
  ```js
  var myFun = function hello() {
      console.log(hello.name)// hello
      console.log(myFun.name)// hello
  ...
  var MyClass = class Me {
      getClassName() {
          ...
          // Me Me
          console.log(Me.name, MyClass.name)
          return Me.name
      }
  }
  var c = new MyClass()
  console.log(c.getClassName(), MyClass.name)// Me Me
  var c1 = new Me()// error
...
- 类的继承
  ...
  - 和 Java 类的继承差不多，都是用 extends 关键字继承
  ...
  // 在子类的构造必须调用 super 继承父类，否则会抛出异常
  // 因为子类中没有 this, 所以要通过执行 super 方法来获得 this 对象
  ...
  var teacher = new Teacher('Bill', 30, 'Maths')
  teacher.toString()
  // "name: Bill, age 30, course: Maths"
  ```

- 原生构造函数的继承

  - js 中原生的构造函数有 Boolean(), Number(), String(), Array(), Data(), Function(), RegExp(), Error(), Object()等，在 ES6 以前是无法直接继承这些原生的构造函数的

  ```js
  // ES5 继承原生构造函数的方法，无法完全正常继承
  function MyArray() {
  ...
  var persons = new Array(1, 2)
  persons[2] = 'Bill'
  persons[3] = 'Mike'
  console.log(persons.length) // 4
  ```
  ...
  - 通过 Class 的继承扩展原生系统的构造函数

  ```js
  // 添加一个方法，计算数组中所有数值类型元素的和
  ...
  var sumArray = new SumArray(1, 2, 'kkk', true, 5)
