	...
	function lookupTemplate(value) {
		...
		if (("" + value === value) || value.nodeType > 0 && (elem = value)) {
			if (!elem) {
				if (/^\.\/[^\\:*?"<>]*$/.test(value)) {
					// tmpl="./some/file.php"
					// If the template is not named, use "./some/file.php" as name.
					if (tmpl = $templates[name = name || value]) {
						value = tmpl;
					} else {
						// BROWSER-SPECIFIC CODE (not on Node.js):
						// Look for server-generated script block with id "./some/file.php"
...
function tmplObject(markup, options) {
	...
	if (!options.phpTag) {
		...
		tmpl.phpTag = htmlTag ? htmlTag[1].toLowerCase() : "";
	}
	htmlTag = wrapMap[tmpl.phpTag];
...
function onRenderError(e, view, fallback) {
	...
	return view && !view.linkCtx ? $converters.php(message) : message;
}
